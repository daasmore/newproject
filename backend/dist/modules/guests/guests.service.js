...
let GuestsService = class GuestsService {
    ...
    async findAllByInvitation(invitationId, query) {
    ...
    async addGuest(invitationId, dto) {
    ...
    async findByToken(token) {
        ...
        if (!guest) {
            throw new common_1.NotFoundException('Tamu tidak ditemukan');
        }
        return guest;
    }
    async submitRsvp(token, dto) {
        const guest = await this.findByToken(token);
        if (guest.rsvpStatus !== 'pending') {
            throw new common_1.ConflictException('RSVP sudah pernah disubmit');
        }
    ...
    async remove(guestId, invitationId) {
        const guest = await this.guestRepository.findOne({
