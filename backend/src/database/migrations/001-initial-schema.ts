...
export class InitialSchema001 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // Indexes
    await queryRunner.createIndex('invitations', new TableIndex({ columnNames: ['user_id'] }));
    await queryRunner.createIndex('invitation_sections', new TableIndex({ columnNames: ['invitation_id', 'type'], isUnique: true }));
    await queryRunner.createIndex('guests', new TableIndex({ columnNames: ['invitation_id'] }));
    await queryRunner.createIndex('messages', new TableIndex({ columnNames: ['invitation_id', 'status'] }));
    await queryRunner.createIndex('orders', new TableIndex({ columnNames: ['user_id', 'status'] }));
  }
